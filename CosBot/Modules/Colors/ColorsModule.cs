...
namespace CosBot.Modules.Colors
{
    ...
    internal class ColorsModule : IModule
    {
            ...
            manager.CreateCommands("colors", group =>
            {
                ...
                group.CreateCommand("set")
                    ...
                    .Do(e =>
                    {
                        ...
                        if (user == null)
                            return _client.ReplyError(e, "Beep Boop. Who is that user?");
                ...
                group.CreateCommand("clear")
                    ...
                    .Do(async e =>
                    {
                        ...
                        {
                            await _client.ReplyError(e, "Beep Boop. I need the Manage Roles permission for this.");
                            return;
                        }
                        var otherRoles = GetOtherRoles(e.User);
                        await e.User.Edit(roles: otherRoles);
                        await _client.Reply(e, $"Beep Boop. Reset username color.");
                    });
            });
        }
        ...
        private async Task SetColor(CommandEventArgs e, User user, string colorName)
        {
            ...
            {
                await _client.ReplyError(e, "Beep Boop. I don't know that color");
                return;
            }
            if (!e.Server.CurrentUser.ServerPermissions.ManageRoles)
            {
            ...
            await _client.Reply(e, $"Beep Boop. Set {user.Name}'s color to {color.Name}");
        }
    }
}
